package com.riprod.hexcode.builtin.glyphs.effect.projectile.system;
...
public class ProjectileSystem extends EntityTickingSystem<EntityStore> {
    ...
    public void tick(float dt, int index, ArchetypeChunk<EntityStore> chunk,
        ...
        ProjectileComponent propel = chunk.getComponent(index, ProjectileComponent.getComponentType());
        ...
        if (state == StandardPhysicsProvider.STATE.INACTIVE) {
            ...
            continueProjectileExecution(signal, resultVar, buffer);
            removeProjectile(projectileRef, buffer);
            return;
        }
        ...
        if (distanceTraveled >= propel.getMaxDistance()) {
            ...
            continueProjectileExecution(signal, null, buffer);
            removeProjectile(projectileRef, buffer);
        }
    }
    ...
    private Ref<EntityStore> findEntityAtPosition(Vector3d position, Ref<EntityStore> casterRef,
        ...
        for (Ref<EntityStore> ref : nearby) {
            ...
            if (ref.equals(casterRef) || ref.equals(projectileRef))
                continue;
            if (buffer.getComponent(ref, ProjectileComponent.getComponentType()) != null)
                continue;
    ...
    private void continueProjectileExecution(HexSignal signal, HexVar resultVar,
