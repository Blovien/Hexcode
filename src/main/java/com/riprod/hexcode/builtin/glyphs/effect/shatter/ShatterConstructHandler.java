...
public class ShatterConstructHandler implements ConstructHandler {
    ...
    private void fireWithResult(HexConstruct construct, ConstructTickContext ctx, HexVar resultVar) {
        if (resultVar != null && construct.getTriggeringGlyph() != null) {
            ctx.fireConditional(hexCtx ->
                    construct.getTriggeringGlyph().writeSlot("result", resultVar, hexCtx));
        } else {
            ctx.fireConditional();
        }
    }
