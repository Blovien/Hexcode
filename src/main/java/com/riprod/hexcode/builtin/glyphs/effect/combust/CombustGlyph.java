...
import com.riprod.hexcode.builtin.glyphs.effect.combust.style.CombustStyle;
...
public class CombustGlyph implements GlyphHandler {
    ...
    public boolean resolveMana(Glyph glyph, HexContext hexContext) {
        ...
        float radiusMultiplier = (float) ((radius * radius) / (DEFAULT_RADIUS * DEFAULT_RADIUS));
        float magMultiplier = (float) (magnitude / DEFAULT_MAGNITUDE);
        float finalCost = baseCost * castMultiplier * radiusMultiplier * magMultiplier;
        ...
        if (!consumed) {
            ...
            LOGGER.atInfo().log("combust: insufficient mana: needs %.1f, has %.1f (base %d, r=%.1f, m=%.1f)",
                    finalCost, currentMana, asset.getManaConsumption(), radius, magnitude);
        }
        return consumed;
    }
