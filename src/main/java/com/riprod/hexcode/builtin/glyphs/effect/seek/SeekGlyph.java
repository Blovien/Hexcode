...
public class SeekGlyph implements GlyphHandler {
    ...
    public void execute(Glyph glyph, HexContext hexContext, ExecutionContext executionContext) {
        HexVar posVar = glyph.getInput(0, executionContext, hexContext);
        HexVar rotVar = glyph.getInput(1, executionContext, hexContext);
        if (rotVar == null || rotVar.size() == 0) {
            rotVar = posVar;
        }

        Vector3d origin = SpellVarUtil.resolveEyePosition(posVar, hexContext.accessor);
        if (origin == null) {
        ...
        Vector3d direction = SpellVarUtil.resolveDirection(rotVar, origin, hexContext.accessor);
        if (direction == null) {
        ...
        int beamLength = (int) SpellVarUtil.resolveNumberOrDefault(
                glyph.getInput(2, executionContext, hexContext), (double) DEFAULT_BEAM_LENGTH).doubleValue();
        ...
        if (posVar instanceof EntityVar entityVar && entityVar.size() > 0) {
            Ref<EntityStore> sourceRef = entityVar.getRef(0, hexContext.accessor);
            if (sourceRef != null && sourceRef.isValid()) {
                entityHit = TargetUtil.getTargetEntity(sourceRef, (float) beamLength, hexContext.accessor);
                if (entityHit != null) {
        ...
        if (entityHit != null && entityHitDistSq < blockHitDistSq) {
            ...
            executionContext.setVariable(outputSlot, resultVar);
