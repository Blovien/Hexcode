package com.riprod.hexcode.builtin.triggers.primary;
...
// permanent bootstrap for the Primary trigger: when fired, scans the player's
// held item for a "Primary" imbuement slot and dispatches the imbued hex.
// countsTowardCharges(false) — imbued casts don't take staff slots.
public final class PrimaryImbuementBinder {

    private static final HytaleLogger LOGGER = HytaleLogger.forEnclosingClass();

    private PrimaryImbuementBinder() {
    }

    public static void register(@Nonnull TriggerListenerRegistry registry) {
        TriggerCallback callback = PrimaryImbuementBinder::onFire;
        registry.subscribe(TriggerSubscription.bootstrap(TriggerKey.PRIMARY, callback));
    }
    ...
    private static void onFire(CommandBuffer<EntityStore> buffer, TriggerSubscription sub, TriggerEvent event) {
        ...
        ItemStack heldItem = ip.itemInHand() != null
                ? ip.itemInHand()
                : com.hypixel.hytale.server.core.inventory.InventoryComponent.getItemInHand(buffer, player);
        if (heldItem == null || heldItem.isEmpty()) return;

        ImbuementData data = ImbuementUtils.read(heldItem, TriggerKey.PRIMARY);
        if (data == null) return;
        ...
        // slot-bound: one Primary cast active per player at a time; new fizzles old.
        castData.setCastSlotKey(TriggerKey.PRIMARY);

        try {
            buffer.invoke(new HexCastEvent(player, castData));
        } catch (Exception e) {
            LOGGER.atSevere().log("Primary imbuement dispatch failed: %s", e.getMessage());
        }
    }
}
