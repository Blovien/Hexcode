...
public class GlyphSelectInteraction extends ChargingInteraction {
    ...
    protected void tick0(boolean firstRun, float time, @Nonnull InteractionType type,
        ...
        if (firstRun) {
            // todo: get player look direction as spherical
            // todo: find glyph under cursor (hit test)
            // todo: set HexcasterComponent.draggingGlyphRef
            LOGGER.atInfo().log("glyph select started");
        }

        // todo: update dragged glyph position based on look direction
    ...
    protected void simulateTick0(boolean firstRun, float time, @Nonnull InteractionType type,
            @Nonnull InteractionContext ctx, @Nonnull CooldownHandler cooldown) {
        // todo: update dragged glyph position to follow look direction (client prediction)
